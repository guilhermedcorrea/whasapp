...
from ..models.leadsNewModel import *
from ..models.usersModel import *

# CONTROLLER LEADS FRANQUEADOS #
# PEGAR TODOS OS LEADS
def get_leads():
    leads = LeadsNew.query.all()
    if leads:
        result = leadsNew_schema.dump(leads)
...
def leadNew_by_telefone(wpp):
    try:
        lead = LeadsNew.query.filter(LeadsNew.bitAtivo, LeadsNew.Celular == wpp).first()
    ...
    if not lead:
        try:
            lead = LeadsNew.query.filter(LeadsNew.bitAtivo, LeadsNew.Celular == utils.fix_number(wpp)).first()
        except Exception as err:
            print(err)
            return None
...
def get_leadNew_by_telefone(wpp):
    lead = LeadsNew.query.filter(LeadsNew.bitAtivo, LeadsNew.Celular == wpp).first()
    if not lead:
        return jsonify({'message': 'Lead não encontrado'}), 404
    result = leadNew_schema.dump(lead)
    print('Object: ', lead)
    print(result)
    return jsonify({'Lead': result})


# ATUALIZA OU RETORNA LEAD
def update_lead(idd):
    lead = LeadsNew.query.filter(LeadsNew.bitAtivo, LeadsNew.IdLead == idd).first()
    body = request.json
    if not lead:
        return jsonify({}), 204
    if lead:
...
def get_lead_by_id(idd):
    lead = LeadsNew.query.filter(LeadsNew.bitAtivo, LeadsNew.IdLead == idd)
    if not lead:
        return jsonify({'message': 'Lead não encontrado'}), 404
    result = leadNew_schema.dump(lead)
